...
namespace RTS_Cam
{
	...
	public class RTS_CameraEditor : Editor
	{
		...
		private void MovementTab()
		{
			...
			camera.translateSpeed = EditorGUILayout.FloatField("Translate speed: ", camera.translateSpeed);
			camera.smoothTime = EditorGUILayout.FloatField("Translate Smooth time: ", camera.smoothTime);
			//camera.velocity = EditorGUILayout.Vector3Field("Cam velocity: ", camera.velocity);
		...
		private void HeightTab()
		{
			...
			if (camera.autoHeight)
			{
				...
				camera.groundMask = EditorTools.LayerMaskField("ground layer", camera.groundMask);
			}
			...
			using (new HorizontalBlock())
			{
				camera.zoomAmount = EditorGUILayout.Vector3Field("Zoom Amount: ", camera.zoomAmount);
