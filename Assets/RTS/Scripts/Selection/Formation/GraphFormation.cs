...
namespace Assets.RTS.Scripts.Selection.Formation
{
	public class GraphFormation : Formation
	{
		...
		public List<Node> Graph { get => graph; set => graph = value; }
		...
		private void GenerateEdges(Node node)
		{
			...
			while (j < rand && j < n - 1)
			{
				Node node2 = Graph[Random.Range(0, n)];
				//Debug.Log(j + " " + n + " "  + (j < n - 1));
